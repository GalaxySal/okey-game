...
const MultiplayerContextInternal = createContext<MultiplayerContextType | undefined>(undefined);
...
export const MultiplayerProvider: React.FC<MultiplayerProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    newSocket.on('player_left', (playerName: string) => {
      setPlayers(prev => prev.filter((p: string) => p !== playerName));
  ...
  return (
    <MultiplayerContextInternal.Provider value={value}>
      {children}
    </MultiplayerContextInternal.Provider>
  );
};

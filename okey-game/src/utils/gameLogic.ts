...
export const analyzeHand = (tiles: Tile[]) => {
  const sortedTiles = [...tiles].sort((a, b) => a.value - b.value);
  const analysis = {
    pairs: [] as [number, number][], // Aynı sayıdan taşlar
    sequences: [] as [number, number][], // Ardışık taşlar
  ...
  for (let value = 1; value <= 13; value++) {
    const sameValueTiles = sortedTiles.filter(tile => tile.value === value && !tile.isJoker && !tile.isOkey);
 ...
 */
const selectBestTileToDiscard = (tiles: Tile[], analysis: { pairs: [number, number][]; sequences: [number, number][]; okeyCount: number; jokerCount: number; totalTiles: number; canFinish: boolean; finishScore: number }, okeyInfo: { indicatorTile: Tile; okeyTile: Tile } | null): Tile | null => {
  ...
  const excessiveTiles = tiles.filter(tile => {
    ...
    return (analysis.pairs.find(([value]) => value === tile.value)?.[1] || 0) > 2;
  });
